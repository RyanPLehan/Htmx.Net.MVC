...
3. Change from Conventional Routing to Attribute based routing
    - See [Routing to controller actions in Asp.Net Core](https://learn.microsoft.com/en-us/aspnet/core/mvc/controllers/routing?view=aspnetcore-9.0)
        - This also removes the dependency of the method name as part of the route in the controller.
    - You may notice that all the controllers, with the exception of the HomeController, have the [ApiController] attribute.  This was done intentionally because, if you think about it, HTMX is using an API for CRUD or Behaviour operations.  Instead of getting the data in a JSON format, the data is HTML fragments.
    - Strict attributes at the Controller, Method and Parameters are introduced.
        - Parameter attributes, (FromRoute, FromQuery, FromBody, etc) to help denote where the parameter values are coming from.  
